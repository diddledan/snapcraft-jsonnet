...
[options.entry_points] section in setup.cfg:
    ...
    console_scripts =
         fibonacci = snapcraft_jsonnet.skeleton:run
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        "--output-file",
        dest="outfile",
        help="set the output file. Should be set to 'snap/snapcraft.yaml' for compatibility.",
        type=str,
        metavar="STR",
    )
    parser.add_argument(
    ...
    if isUrl:
        with urllib.request.urlopen(full_path) as f:
            if (f.getcode()==200):
                ...
                _logger.error("Error receiving remote {url}".format(url=full_path),
                    f.getcode())
